...
**To meet.** Justify or empirically derive weights and cutpoints; test compensatory vs. conjunctive combination (monotonicity check now run). Re-label tiers as an intensity/heat scale rather than a quality scale — a shippable, stranger-verifiable fix. Any re-weighting is a new pre-registered hypothesis judged OOS (P5), never fit to this table in-sample.
