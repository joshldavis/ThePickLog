...
try:
    ...
    from hypo_eval import (_keeps as _hp_keeps, _exit_return as _hp_exit,
                           _cluster_paired_ci as _hp_cluster, _ticker_signs_paired as _hp_signs)
    ...
    _hp_keeps = _hp_exit = _hp_cluster = _hp_signs = None
    _SEL = {}
...
def main():
    all_picks = _read(os.path.join(HERE, "picks.csv"))
    all_outs = _read(os.path.join(HERE, "outcomes.csv"))
    # COHORT SEAL (H-UNIV1, 2026-07-08): every section below reports the v0.2
    # fixed-16 record — the closed cohort all current registrations were frozen
    ...
    # ---- 6. v0.3 market-wide cohort (young; separate record) ----
    v3_picks = [p for p in all_picks if (p.get("model_version") or "").startswith("v0.3")]
    w("## 6. v0.3 market-wide cohort (H-UNIV1 — registered 2026-07-08)")
    w("")
    if not v3_picks:
        w("- No v0.3 picks logged yet. The criteria-defined market-wide scan (Alpaca "
          "screener; frozen gates in universe.py) starts logging once the Alpaca data "
          "keys are live. This cohort is scored with the unchanged v0.2 formula but is "
          "a SEPARATE record: none of the tables above include it, and no v0.2 "
          "hypothesis verdict reads it.")
    else:
        v3_ids = {p["pick_id"] for p in v3_picks}
        v3_outs = [o for o in all_outs if o.get("pick_id") in v3_ids]
        v3_dates = sorted({p["trading_date"] for p in v3_picks})
        v3_tickers = {p["ticker"] for p in v3_picks}
        rets = [x for x in (col(o, "ret_open_close_net") for o in v3_outs) if x is not None]
        w(f"- **{len(v3_picks)} picks** across **{len(v3_dates)} scan days** "
          f"({v3_dates[0]} → {v3_dates[-1]}); **{len(v3_tickers)} unique tickers** "
          f"(the H-IND1 effective-N readout the v0.2 cohort lacked).")
        if rets:
        ...
        w("- Candidate pool + non-published eligibles: `candidates.csv` "
          "(forward-only control pool for H-CTRL).")
        w("- Young cohort — directional only; no v0.3 rule is registered yet, so "
          "nothing here feeds any verdict.")
    w("")
    w("---")
