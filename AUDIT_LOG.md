...
**Verification.** The two edited template literals were extracted from `index.html` by line and evaluated in Node with a stub `tr`; both render and both new phrases are present. `git diff --stat` before staging: `index.html | 4 ++--`, nothing inherited from another session.

**The 09-14 skipped session — explained, and already fixed by the time it was flagged.** The Actions history for 09-14 shows twelve runs, every one `Scheduled`: the GitHub cron ladder was the only trigger that day, and its first rung (`0 07 UTC`) arrived at **13:40Z — 6h40m of drift against a ladder that tolerates ~6h20m**. The 09-12 cron-drift note predicted this failure in those exact terms ("luck-adjacent") and named the remedy: the Vercel Cron → `workflow_dispatch` trigger, which was inert for want of two env vars. The timeline in the Actions log settles when that changed: 09-11, 09-14 and 09-15 have only `Scheduled` runs (first rungs at 12:02Z, 13:40Z, 12:28Z — inside, missed, inside); from **09-16** the first run of every session is **"Manually run by joshldavis" at 11:56Z (07:56 ET)** — Vercel's cron calling GitHub with the fine-grained PAT — and `/api/cron-scan` now answers 401 rather than the old 500 `missing: [GITHUB_DISPATCH_TOKEN, CRON_SECRET]`. The GitHub rungs behind it have been landing 4–5.5h late all week; without the Vercel dispatch, 09-22 would have been logged at 08:22 ET by the one rung that squeaked in, and any morning with 09-14's drift would be lost. So 09-14 was the last session exposed to the ladder alone, and no session since has depended on it. Two residual checks, both clear: `SCAN_FLOOR` is inclusive (`now < floor` refuses), so a Vercel fire at exactly 11:00Z in winter (06:00 EST) still logs; and 11:56Z sits at 07:56 EDT / 06:56 EST, inside the window under both clocks. Vercel's own invocation logs are not retrievable on this plan (`ExceedsBillingLimitError`), so the evidence is the GitHub event type, which is sufficient.
...
Every published figure re-derives from the two CSVs, and the chip reconciliation added on 09-15 holds. No failures; two observations, neither of which moves a number. **One new skipped session (2026-09-14) — the gate refused it, the ledger recorded it, and the page names it. That is the system working, but it is the same scheduler-lateness failure mode as the 08-27 → 09-03 outage, recurring after ten clean sessions.**

- **Data served.** `/picks.csv` 200, 235,620 bytes, 1,335 rows. `/outcomes.csv` 200, 154,760 bytes, 1,236 rows. Every static page 200, and every *rendered* anchor on the homepage resolves — 1,097 hrefs, including the 1,074 `/r/p-<pick_id>` receipt routes, all 200. (Checking rendered anchors rather than `href="..."` in the source matters: the source-level scan surfaces five template-literal fragments like `${co}` that are not links at all.)
- **Real data shown.** Track record renders 1,185 real rows through **2026-09-18**. No sample fallback — the only occurrence of "sample" on the page is "out-of-sample" in the pricing block.
- **No silent gaps.** 65 distinct `trading_date`s, 06-09 → 09-18, 64 of them sessions. Seven weekdays are missing and **all seven are declared on the page and in `skipped_sessions.csv` with a timestamp and reason** — the six known outage sessions (08-27, 08-28, 08-31, 09-01, 09-02, 09-03) plus the new **09-14**. The 06-19 Juneteenth cohort remains the sole holiday cohort, still `VOID` and still excluded from every figure; 07-03 and 09-07 are correctly absent. Recent cohorts publish at **07:17–08:29 ET** — above the 06:00 `SCAN_FLOOR`, clear of the 09:20 cutoff. 09-15 at 08:29 is the latest of the ten and still 51 minutes inside it.
- **Claims == data.** Recomputed independently; every displayed figure matches to the precision shown. Picks logged **1,185**, graded **1,074**, win rate **33%** (33.05 — 355 W / 719 M), median net **−2.7%** (−2.70), mean net **−2.4%** (−2.38), avg worst dip **−17.8%** (−17.84). Chips reconcile exactly: **1,074 + 87 + 24 = 1,185**. Tier row counts A 330 / B 79 / C 306 / D 359 = 1,074. The overdue-pick note — "1 pick is more than three sessions past the grading window (A: 1)" — reproduces from its own `>5 + AWAIT_SLACK` rule: the 09-04 A-tier row, ten sessions on.
- **Exclusions reproduce.** All three re-derived from scratch: **128 late picks across 7 cohorts**, per-date counts identical to the on-site note (06-15 13, 07-06 16, 07-07 16, 07-09 21, 07-13 21, 07-27 22, 08-03 19); **12 frozen-quote (BNZI)**; **10 scale-mismatch** with the same ticker tally (VMAR 7, SUGP 1, CPHI 1, SLE 1). Union 150; 1,335 − 150 = 1,185. See observation 1 — the frozen-quote *count* is right, but the published *description* of the rule does not reproduce it.
- **Honest grading.** All **1,211** rows carrying data reproduce open→close minus the 2% haircut to within 0.06pp (0 exceptions). Zero duplicate `pick_id`s in either file, zero outcomes without a parent pick, `win` consistent with the sign of the net on all 1,211. The 25 ungradeable rows all carry an explicit note — 14 `VOID: phantom scan — NYSE closed`, 9 `no entry bar`, 2 `UNGRADEABLE: only N of 6 sessions ever printed`. Zero empty notes.
- **Disclaimers.** Educational / not advice / not a registered investment adviser or broker-dealer present on **all three** of index, method and disclaimer. (index carries it at line 790, "broker-dealer or registered investment adviser" — the 09-15 method.html fix is still in place.)

**Six-aspect validity check — all honest.** All six Messick aspects are stated on method.html §9; the three validity docs return 200 and are linked from it (plus two more: `-Empirical-Validity-Studies-` and `-Validity-Dossier-UG15-`). The model carries the **unvalidated** label — 15 rendered instances on index, now generated from a single `cvconf` template rather than hardcoded, plus method.html §9. Gate-1's external-validity verdict is not rendered here; it belongs to `thepicklog-gate1-external-validity-verdict`.

**Structural — re-checked from the CSVs, not from memory.** Tiers render as intensity ("heat scale, not a quality grade"; "the hottest tiers show the deepest drawdowns"), and §9 still quotes no tier return figures. Its two standing assertions both verify: **both tails widen monotonically A→D** — mean MAE **−22.97 / −19.31 / −15.55 / −14.76**, mean MFE **+41.20 / +35.88 / +19.65 / +15.88** — and **all six pairwise 95% CIs on mean net overlap** (A −1.56, B −2.42, C −3.12, D −2.48), so no ordering is established. A is again best of the four on mean net this week; that is exactly why §9 asserts no ordering instead of naming one.

**Observation 1 — the frozen-quote exclusion is correct but not reproducible from what the page says it does.** The on-site note invites the reader to re-derive the set from repeated `price_at_screen` / `gap_pct` / `rvol` / `float_shares` tuples. Do that literally and you get **22 rows, not 12**. The implementation additionally drops non-session dates (`_isSession`) *before* grouping, and the note never says so. The 10 extra rows are five tickers screened twice — **BJDX, CUPR, GCDT, IOTR, PW, each on 06-19 and again on 06-22, byte-identical quote both times** (e.g. CUPR $4.37 / gap −16.92% on both). Because one twin sits on Juneteenth, each group drops to a single session and the guard sees nothing. So the five **06-22** rows — screened on a quote that had not moved since before a market holiday, which is precisely the defect the seal exists to catch — remain in every published figure. Impact is sub-rounding and runs *against* us: all five are losses, and removing them would move mean net −2.38 → −2.36, median −2.70 → −2.69, win rate 33.05% → 33.21%. Nothing published is wrong. What is wrong is that a stranger following the site's own instructions gets a different answer than the site does. **Fix:** state the session filter in the note (one clause: "…on any two *sessions*; rows dated to a market holiday are void already and are not grouped"), and decide explicitly whether a live-session row whose twin is a void holiday row should be sealed — the honest answer is probably yes, since the evidence of a frozen feed is the same evidence.

**Observation 2 — "Our scheduler fired hours late" is false for one of the seven sessions it covers.** The missing-sessions sentence generalises across all seven, and for six of them it is accurate (08-27 was +439 minutes). But **09-14's scan started 09:40:51 EDT — 20 minutes past the 09:20 cutoff, +11 minutes relative to the open**, per the `detail` column of the very file the sentence points the reader at. This is the "55 trading days" defect again in miniature: a falsifiable sentence sitting inside the invitation to check our arithmetic. **Fix:** drop "hours" ("Our scheduler fired late, so the pre-open gate refused to log anything"), or better, derive the lateness per session from `skipped_sessions.csv` the way the counts already are.

**Verification.** Site logic was not retyped — `staleQuoteIds` and `scaleMismatchIds` were **extracted from the served `index.html` and executed against the live CSVs**, so the exclusion sets were reproduced by the code that actually ships; the from-scratch re-derivations were run separately and the 22-vs-12 divergence above is the difference between the two. No front-end changes were made this run, so nothing was deployed. Repo clean at `e104219` before and after; `gh` is installed and authed as `joshldavis` (no issue opened — no check failed).

**Grading in the coming week.** 87 picks pending. **09-15 (21) grades ~09-22, 09-16 (20) ~09-23, 09-17 (19) ~09-24, 09-18 (21) ~09-25**, each once its five-session window closes. Six older stragglers remain — 09-04 (1), 09-08 (3), 09-11 (2); the 09-11 pair should clear this week, and the 09-04 and 09-08 rows are past the window and will most likely resolve as halts or delistings with an explicit `UNGRADEABLE` note rather than a return.
...
**2. Scan-time detector (09-05 Issue 1, prescription 2) — shipped as a WARNING, not a filter, and the prescription is respectfully narrowed.** `is_corporate_action()` reports when a name's float re-scales while its price stays essentially flat: a real corporate action big enough to move float by >2× necessarily moves price too, so a flat price beside a re-scaled float means the two feeds are not describing the same instant. VMAR 08-26 is detected (`float moved 0.100x … price barely moved (1.014x)`); VMAR 08-17 is not.

**It does not drop the candidate, because the data says it must not.** The prescription was to *refuse* such a candidate. Replaying the rule over the whole live log before shipping it showed it would have removed **11 picks** (16 under a `float × price` variant) against exactly **one** genuine defect — NCT 3.49×, BJDX 3.36×, JAGX 8.57× and the rest are simply names whose *reported* float was revised between sessions, because `float_shares` is approximated from shares outstanding and is a noisy series. **Dropping those would be a selection distortion: the same harm class as the contamination it was meant to catch, and the same mistake the frozen-quote guard made on 09-03 when it destroyed a valid cohort.** The precise instrument is the read-time exclusion above, which reconciles against a genuinely independent observation (`entry_open`) and selects the ten contaminated rows with no false positives. The detector's job is to make the condition visible in the run log on the morning it happens instead of a week later in an audit — it now emits ~12 notes across the entire log and removes nothing.

⭐ **Generalisable lesson: replay a proposed filter over the existing record before you let it remove anything.** The measurement takes minutes and is the difference between a guard and a leak. Both defects this month came from filters that were never replayed.
...
**Issue 1 — the Issue-3 fix was applied to one page, not to the claim.** On 2026-08-29 the tier caption on index.html was correctly rebuilt to *derive* its ranking claim from the same `tierStats` that renders the table, and it reads correctly today: *"If the score ranked edge, mean net would fall in tier order here — it doesn't."* True — A −1.6% > B −1.9% > C −3.1% < D −2.5% is not monotone. But **method.html §9 still hardcodes the sentence the fix was written to eliminate**: *"the top tier historically has the worst mean net and the deepest drawdowns."* Recomputed from the live CSVs, the two halves now disagree:

| tier | n | mean net | median net | avg worst dip (MAE) | avg best excursion (MFE) |
|---|---|---|---|---|---|
| A | 293 | **−1.61%** | −3.24% | **−23.30%** | +43.10% |
| B | 69 | −1.88% | −3.57% | −19.16% | +36.79% |
| C | 296 | −3.14% | −2.35% | −15.45% | +20.99% |
...
**"Deepest drawdowns" is not merely still true — it is perfectly monotone A→D, and so is MFE.** That is the H-RISK finding rendered cleanly: the score predicts *magnitude*, not direction. **"Worst mean net" is false**: A is now the *best* of the four on mean net, and C is the worst. As on 08-29, the error runs self-critically — the site is harder on itself than its data supports, so no reader is pushed toward the product — but it is a sentence a stranger recomputing from `picks.csv` + `outcomes.csv` can falsify, which is exactly the North Star test.

Prescribed fix — and it must not flip into "A is better", which the data does not support either. Replace the §9 clause with the two claims that *are* backed: **the top tier has the deepest drawdowns (mean MAE −23.3% vs −14.2% for D, monotone across all four tiers) and the worst median net (−3.2% vs −2.7% for D)** — A's mean is lifted by a right tail (MFE +43.1%), and its clustered interval `[−3.7%, −0.0%]` overlaps every other tier, so no tier ordering on mean net is established in either direction. Better still, make §9 read the same computed values index.html already has rather than restating them by hand.

⭐ **Generalisable lesson: fixing the instance an audit sampled is not fixing the defect.** The 08-29 entry named the failure mode precisely — *"a hardcoded ranking claim under a live table will always eventually invert"* — and the fix that shipped derived exactly one such claim. The remedy for a class of stale prose is a repo-wide grep for the class, not a patch at the coordinates the auditor happened to quote. This is the fourth appearance of the standing rule that **prose asserting a statistical fact must be COMPUTED from the data under it** (08-19 UI audit, 08-29 Issue 3, and now its own incomplete remediation).

*Cosmetic, not an issue:* the derived caption says *"Only A and C and D separate from zero at all, and both are significantly negative"* — "both" for three tiers. The generator pluralises for n=2 only.

Data checks (recomputed in-browser from the live /picks.csv + /outcomes.csv, independently of the page's own functions):

- **Data served:** picks.csv **200, 1,169 rows**; outcomes.csv **200, 1,134 rows**. ✅
- **Real data shown:** Track record renders **983 real graded rows** through 2026-08-26 with real tickers and prices. Zero sample-fallback badges anywhere on the page (the only "sample" string is the pricing block's "out-of-sample", as expected). ✅
- **No silent gaps:** **57 distinct trading dates, 2026-06-09 → 2026-09-04.** Six US market weekdays are absent — **08-27, 08-28, 08-31, 09-01, 09-02, 09-03** — and **all six are declared, not silent**: each carries a row in `skipped_sessions.csv` with its reason and minutes-past-cutoff, and index.html states verbatim *"6 sessions missing from the log entirely… Those sessions are simply gone. We count them here because a record that quietly skips the days it failed on is not a record."* This is the loud-failure design behaving exactly as intended. Every other market weekday since 06-09 is present (Juneteenth 06-19 and observed 07-03 excepted). One cohort sits on a holiday — 06-19, the known phantom scan — and **all 14 rows carry the VOID note**, so none sit orphaned in the ungraded pool. Late-cohort derivation reproduces exactly from `published_at`: **7 cohorts / 128 picks**, unchanged for a fifth week. ✅
- **The scan is alive.** **2026-09-04 logged 22 picks, timely** — the first successful cohort since 08-26 and the first evidence that `7452e64` (Vercel Cron trigger + laddered backup) and `3d8dc27` (the frozen-quote guard fixed and made non-fatal) both hold in production. The six-session outage is closed.
- **Claims == data (timely, non-stale, non-VOID):** picks logged **1,029** ✅ (1,169 − 128 late − 12 frozen-quote) · graded **983** ✅ · blended win rate 33.06% → **"33%"** ✅ · median net −2.69 → **"−2.7%"** ✅ · mean net −2.37 → **"−2.4%"** ✅ · avg worst dip −17.65 → **"−17.7%"** ✅. Best single open→close net **+209.56%**; best 5d-swing net **+661.22%**; mean 5d-swing net −5.93%. Tier table exact on every column: **A 293/224/−1.6%/−3.2%/[−3.7%,−0.0%] · B 69/43/−1.9%/−3.6%/[−4.7%,+0.7%] ns · C 296/18/−3.1%/−2.4%/[−4.1%,−2.4%] · D 325/16/−2.5%/−2.7%/[−3.2%,−1.7%]** ✅ (tiers sum to 983, reconciles exactly). The frozen-quote exclusion still derives to **12 rows**, byte-identical to the set published on 08-29 — `3d8dc27` moved no published number, as that commit claimed. ✅
- **Honest grading:** **400 of 400** graded rows recompute to their stored `ret_open_close_net` to within 0.02pp under `(close − open)/open × 100 − 2` — entry = open, 2% cost haircut, no exceptions. Spot-checks: TNMG 08-26 (+4.81), AMIX 08-26 (−8.59), EXOD 08-26 (−12.19), all exact. **Zero duplicate pick_ids in either CSV, zero regrades, zero orphan outcome rows, and zero rows where the stored `win` flag disagrees with the sign of the net return** across all 1,134 outcomes. ✅
...
- **Validity (six aspects):** (a) **structural — tiers still framed as intensity, PASS.** Every tier reference on the site reads as heat, never quality — *"an intensity ranking, not a quality grade"*, *"how hot the setup is, not how good"*, *"treat a high tier as a downside warning, not a buy"* — and **nothing anywhere implies higher tier = better**. The drawdown evidence behind that framing is not just intact but has strengthened to perfect monotonicity (MAE −23.3 / −19.2 / −15.5 / −14.2; MFE +43.1 / +36.8 / +21.0 / +16.3). The stale *supporting* sentence on method.html is Issue 1 — a disclosure defect, not a reversal of the finding. (b) **external — labelling honest:** the page still leads with "FAILED" on Experiment 01, and "unvalidated" is carried **15× on index and 2× on method** for the model itself ✅. (c) all three Messick docs return **200** and all three are **linked from method.html §9** ✅. Content, substantive, generalizability and consequential claims unchanged from the standing rubric and still either evidence-backed or labelled. ✅

**Grading forward.** The ungraded pool is **59 rows**, of which **37 are permanently excluded and correctly disclosed** — 14 VOID (06-19 phantom holiday), 12 frozen-quote BNZI rows, and 11 further single rows from halted names between 07-16 and 08-06 that carry VOID or are covered by the derived exclusion. That leaves **22 live rows: the 2026-09-04 cohort**, which needs five sessions (09-08, 09-09, 09-10, 09-11, 09-14 — **Labor Day 09-07 is closed**) and should grade **Mon 2026-09-14 or Tue 2026-09-15** on the observed six-session lag. **No cohort grades between now and then**, because 08-27 → 09-03 produced nothing to grade — the outage propagates forward one more week before the record resumes.
